...
#include <my_global.h>
//incluir esta librerÃƒÂ­a para poder hacer las llamadas
#include <string.h>
#include <mysql.h>
#include <time.h>
#include <sys/types.h>
...
typedef struct {
 ...
 typedef struct {
 	int num; // numero de usuarios en la lista
 	Tusuario usuarios [MAX]; // defino a una variable char llamada "usuarios" de tipo Tusuario
 } Tlista;

//.........Fin de las estructuras........


 /* No deberÃƒÂ­amos crear una lista local por Thread. hay que arreglarlo*/
Tlista mi_lista;
Tusuario t;
...
{
	     int err;
              /* MYSQL *conn; //Conector para poder acceder a la base de datos

			conn = mysql_init(NULL); //Creamos una conexiÃƒÂ³n al servidor MYSQL
			...
			//Inicializar la conexiÃƒÂ³n, indicando nuestras claves de acceso
			...
			exit (1);
              ...
              ###EL CONECTOR ES UNICO SE CREA EN CONEXION SQL TODO ESTO QUE ESTA COMENTADO FUERA###*/

			/*Ahora creo la base de datos, que se llamarÃƒÂ¡ jugadoresPartidas
 ...
 int conexion_a_sql() // Conectamos con sql
 {
	 conn = mysql_init(NULL);
	 	//Creamos una conexiÃƒÂ³n al servidor MYSQL
	 	if (conn == NULL) {
	 		printf("Error al crear la conexiÃƒÂ³n: %u %s\n", mysql_errno(conn),
	 				mysql_error(conn));
	 		exit(1);
...
int new_user(char buf[MAX])
{
    ...
    sscanf (buf,"%d  %s  %s",&code, user, password); // Cargamos lo que nos llega del cliente a traves de la variable buf a las variables code, user y password
    printf("%s\n\n",buf);
    sprintf(consulta,"INSERT INTO Jugador (Nombre,password) VALUES ('%s','%s')",user,password); //insertamos los valores que hemos cargado en la variable user y password en la base de datos
    printf("%s\n",consulta);
    printf("%s\\ %s \n\n",user,password);
       ...
       printf("%d",err);
	   if(err!=0){
                 printf("Se ha producido algun error en la introducciÃƒÂ³n de los datos %u %s \n",mysql_errno(conn),mysql_error(conn));
                  return -1;
	              exit(1);
              }
	   ...
	   {
	         return 0;
	         printf("Hemos aÃƒÂ±adido al usuario correctamente \n");
...
char* lista_fecha(char buf[MAX])
		   ...
		   // msg= &mensaje; WTF???
			...
			sprintf(consulta,"SELECT fecha FROM Partida ",user);
					...
					printf("err %d \n",err);
						...
						// Evitamos que nos salgan caracteres 'raros' cuando concatenamos por 1Ãƒâ€šÃ‚Âª vez.
						strcpy(mensaje,"");
  ...
  void inicializa_lista (Tlista *l) // le paso como referencia una variable tipo puntero llamada l que apunta a la estrictura Tlista, me servira como variabla para poder guardar lo que yo quiera dentro de la estructura.
  ...
  /* AÃƒÂ±ade el usuario u al final de la lista.
 ...
 void *usuarios(void *conector)
  {
       ...
       /*Haciendo la conversiÃƒÂ³n del SocketID pasado por argumento*/
       con = (int) conector;
       ...
       /*----Falla!---*/
       /* ---Falla en este punto cuando se introduce un segundo usuario y se
       valida, ya que no se guarda correctamente en la lista, porque se salta esta linea.---*/
       ...
       switch(code){
	      ...
	      case 2:
		     ...
		     if(resultado==0){
	            ...
	            case 3: // Listamos a los jugadores conectados.
...
int main(void) {
    ...
    for(;;){
       ...
       /*Incrementamos el numero de Threads usados*/
       T++;
       } //fin del WHILE
    } // fin del FOR
pthread_exit(NULL);
